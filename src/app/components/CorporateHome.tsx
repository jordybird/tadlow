...
const CorporateHome = () => {
  ...
  // Images array with corrected alt texts
  const images = [
  ...
  // Handle slide transitions without animations
  const goToSlide = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      goToSlide();
    }, 3000); // Set to 3 seconds; adjust as needed for faster/slower transitions

    return () => clearInterval(interval);
  }, [isMobile]); // Only depend on isMobile to prevent interval reset on slide change
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-2 gap-8 md:gap-16 items-start px-4 md:px-0">
      ...
      {/* Desktop version with offset images */}
      {!isMobile && (
      ...
      {isMobile && (
        <div className="mobile-slideshow relative w-full h-[400px] md:hidden">
          ...
          >
            <Image 
              key={images[currentSlide].src} // Ensures Image re-renders on slide change
